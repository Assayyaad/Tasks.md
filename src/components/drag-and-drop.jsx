...
function Target() {
  ...
  const draggableItem = createMemo((prev) => {
    ...
    const targetComputedStyle = window.getComputedStyle(dragAndDropTarget().originalElement)
...
function Container(props) {
  ...
  function handlePointerUp() {
    setTargetBeforeMoving(null)
    setStartPageCoordinates(null)
    if (!dragAndDropTarget().originalElement || !sortedItemsIds || dragAndDropTarget().to !== props.id) {
      return
    }
    const index = sortedItemsIds.findIndex((id) => id === dragAndDropTarget().originalElement.id)
    props.onChange({
  ...
  function sortItems(direction) {
    ...
    const targetPosition = dragAndDropTarget()[positionProperty()] + containerRef[scrollProperty()]
  ...
  createEffect((prev) => {
    ...
    if (props.id !== dragAndDropTarget().to) {
      setSortedItemsIds(sortedItemsIds.filter((id) => id !== dragAndDropTarget().originalElement.id))
      return JSON.stringify(dragAndDropTarget())
    }
    if (props.id !== dragAndDropTarget().from) {
      const endPadding = flexDirection === 'row' ? 'paddingRight' : 'paddingBottom'
      containerRef.style[endPadding] = `${dragAndDropTarget()[lengthProperty()]}px`
    }
    if (!sortedItemsIds.length) {
      const newSortedItemsIds = items().map((item) => item.id)
      setSortedItemsIds(newSortedItemsIds)
    }
    if (!sortedItemsIds.some((id) => id === dragAndDropTarget().originalElement.id)) {
      const targetId = dragAndDropTarget().originalElement.id
      setSortedItemsIds(sortedItemsIds.length, targetId)
    }
    let direction
    if (JSON.parse(prev).originalElement !== null) {
      direction = Math.sign(dragAndDropTarget()[positionProperty()] - JSON.parse(prev)[positionProperty()])
    }
  ...
  createEffect((prev) => {
    ...
    const isBeforeEndEdge =
      dragAndDropTarget()[outerDirection] + dragAndDropTarget().originalElement[outerClientLength] / 2 <=
      containerEdgeEnd
  ...
  createEffect((prev) => {
    ...
    const isDescendant = containerRef.contains(dragAndDropTarget().originalElement)
    if (!isDescendant && !isSameGroup) {
      return
    }
    const itemLength = dragAndDropTarget().originalElement[clientLengthProperty()]
    const maxScroll = Number.MAX_SAFE_INTEGER
    ...
    if (
      ...
      dragAndDropTarget()[positionProperty()] + itemLength * autoscrollThreshold >= containerEndPos
    ) {
