...
function ExpandedCard(props) {
  ...
  function handleTagRenameChange(newValue) {
    ...
    const taskAlreadyHasThisTag = props.tags.some((tag) => tag.name.toLowerCase() === newTagName().toLowerCase())
  ...
  function handleTagRenameConfirm() {
    ...
    const emptyLineIfFirstTag = [...actualContent.matchAll(/\[tag:(.*?)\]/g)].length ? '' : '\n\n'
  ...
  function deleteTag(tagName) {
    ...
    let indexOfTag = currentContent.toLowerCase().indexOf(tagWithBracketsAndSpace)
    if (indexOfTag === -1) {
      indexOfTag = currentContent.toLowerCase().indexOf(tagWithBrackets)
      tagLength += 1
    }
    const newContent = `${currentContent.substring(0, indexOfTag)}${currentContent.substring(
      indexOfTag + tagLength,
  ...
  function getButtonCoordinates(event) {
    ...
    const { x: dialogX, y: dialogY, height: dialogHeight, width: dialogWidth } = dialogCoordinates
  ...
  const colorMenuOptions = new Array(tagOptionsLength).fill(1).map((option, i) => ({
    label: (
      <>
        Color {i + 1} <div class="color-preview-option" style={{ 'background-color': `var(--color-alt-${i + 1})` }} />
      </>
    ),
  ...
  onMount(() => {
    ...
    const toolbarEndGroupNodes = [
      ...editorContainerRef.childNodes[0].childNodes[1].childNodes[0].childNodes[1].childNodes[0].childNodes
    ]
  ...
  return (
    <Portal>
      <div
        ...
        onKeyDown={(e) => handleKeyDown(e, (event) => handleBackdropClick(event))}
        ref={(el) => {
          backdropRef = el
        }}
      >
          ...
          <div class="dialog__body">
            <header class="dialog__toolbar">
              ...
              <div class="dialog__toolbar-btns">
                ...
                <button type="button" class="dialog__toolbar-btn" onClick={props.onClose} title="Close">
              ...
              <div class="dialog__tags">
                {isCreatingNewTag() ? (
                  <NameInput
                    ...
                    datalist={
                      <datalist id="tags">
                        <For each={availableTags()}>{(tag) => <option value={tag.name} />}</For>
                      </datalist>
                    }
                  />
                ) : (
                ...
                <For each={props.tags || []}>
                  {(tag) => (
                    <div
                      ...
                      onKeyDown={(e) => handleKeyDown(e, () => handleTagClick(e, tag))}
                      tabIndex={0}
                    >
                      <h5>{tag.name}</h5>
                    </div>
