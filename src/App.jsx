...
import { CardName } from './frontend/src/components/card-name'
import { makePersisted } from '@solid-primitives/storage'
import { DragAndDrop } from './frontend/src/components/drag-and-drop'
...
function App() {
  ...
  const board = createMemo(() => {
    ...
    if (pathname.endsWith('.md') || pathname.endsWith('.md/')) {
      ...
      pathnameParts.pop()
      const concatenatedName = pathnameParts.join('/').substring(basePath().length, pathname.length)
      if (!concatenatedName) {
        return ''
      }
  ...
  async function fetchData() {
    ...
    const newLanes = lanesFromApi.toSorted((a, b) => lanesSortedKeys.indexOf(a) - lanesSortedKeys.indexOf(b))

    let newCards = resources.map((resource) => resource.files.map((file) => ({ ...file, lane: resource.name }))).flat()
    ...
    const tagsWithColors = localTagNames.map((tagName) => {
      ...
      const tagColor = remoteTag?.backgroundColor || getTagBackgroundCssColor(pickTagColorIndexBasedOnHash(tagName))
      return {
        name: tagName,
    ...
    newCards = newCards
      .map((card) => {
        ...
        newCard.tags = tagsWithColors.filter((tagOption) => cardTagsNames.includes(tagOption.name))
  ...
  async function changeCardContent(newContent) {
    ...
    const newCardIndex = structuredClone(
      newCards.findIndex((card) => card.name === selectedCard().name && card.lane === selectedCard().lane)
    )
    ...
    const cardTagOptions = cardTagsWithoutDuplicates.map((tagName) => {
      ...
      const tagColor =
        remoteTagOption?.backgroundColor || getTagBackgroundCssColor(pickTagColorIndexBasedOnHash(tagName))
      return {
        name: tagName,
        backgroundColor: tagColor
      }
  ...
  function getTagsByCardContent(text) {
    const tags = [...text.matchAll(/\[tag:(.*?)\]/g)].map((tagMatch) => tagMatch[1].trim()).filter((tag) => tag !== '')
    return tags
  }
  ...
  const filteredCards = createMemo(() =>
    sortedCards()
      ...
      .filter(
        (card) =>
          filteredTag() === null ||
          card.tags?.map((tag) => tag.name?.toLowerCase()).includes(filteredTag().toLowerCase())
      )
  )
  ...
  function handleLanesSortChange(changedLane) {
    ...
    setLanes([...newLanes.slice(0, changedLane.index), lane, ...newLanes.slice(changedLane.index)])
  }
                          ...
                          cardBeingRenamed()?.name === card.name ? (
                            <NameInput
                              ...
                              errorMsg={validateName(
                                newCardName(),
                                cards()
                                  .filter((card) => card.name !== cardBeingRenamed()?.name)
                                  .map((card) => card.name),
                                'card'
                              )}
                            ...
                            <CardName
                              ...
                              onClick={() => navigate(`${basePath()}${board()}/${card.name}.md`)}
                            />
                          )
                        }
                      />
